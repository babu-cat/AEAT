  ...
  public function getDeductionPercentAndDonationsRecurrence($contactType, $amountThisYear, $amountLastYear, $amountTwoYearBefore, $cp, $euro = TRUE) {
    ...
    if ($contactType == self::NATURAL_PERSON) {
      ...
      if ($amountThisYear <= 150) {
        $deducted_amount_max = floatval($amountThisYear) * intval($max_current_deduction) * 0.01;
        $deducted_amount_min = floatval($amountThisYear) * intval($min_current_deduction) * 0.01;
      }
      else{
      ...
      if ($amountThisYear <= 250) {
        $deducted_amount_new_max = floatval($amountThisYear) * $max_current_deduction_new * 0.01;
        $deducted_amount_new_min = floatval($amountThisYear) * $min_current_deduction_new * 0.01;
      }
      else{
        $partial_amount_new = $amountThisYear - 250;
        if ($donationsRecurrence == 1) {
          $min_new_deduction = '45';
        }
        else {
          $min_new_deduction = '40';
        }
      ...
      if ($this->autonomousDeduction && self::isAutonomousCommunityProvince(substr( $cp, 0, 2 ),self::ACC_CATALONIA)) 
      {
        ...
        $deducted_amount_new_min += floatval($amountThisYear) * 15 * 0.01;
        $deducted_amount_new_max += floatval($amountThisYear) * 15 * 0.01;
    ...
    elseif ($contactType == self::SOCIETIES) {
      if ($donationsRecurrence == 1) {
        ...
        $deducted_amount_min = $deducted_amount_max = floatval($amountThisYear) * 35 * 0.01;
      }

      //Bloque relativo a la nueva normativa para 2024 de personas jurídicas
      if ($donationsRecurrence == 1) {
        $min_new_deduction = '50';
        $deducted_amount_new_min = $deducted_amount_new_max = floatval($amountThisYear) * 50 * 0.01;
      }
      else {
        $min_new_deduction = '40';
        $deducted_amount_new_min = $deducted_amount_new_max = floatval($amountThisYear) * 40 * 0.01;
      }    
    // Fin bloque relativo a la nueva normativa para 2024 de personas jurídicas
      ...
      if ($amountThisYear <= 150) {
        $contribution_new_min = $contribution_new_max = (($deducted_amount_new_min - $deducted_amount_min)*100)/(100-$new_deduction_min);
      }else{
    ...
    elseif ($contactType == self::SOCIETIES) {
      ...
      $diff_min = $deducted_amount_new_min - $deducted_amount_min;
      $contribution_new_min = $contribution_new_max = ($diff_min * 100)/(100-$min_new_deduction);
      // Fin bloque relativo a la nueva normativa para 2024 de personas jurídicas
    }
